...
show_help() {
    cat <<EOT
Usage: ${PROGNAME} NAME
    NAME must be correspond to the files generated by 'make-csr' :
    - a CSR in ${CSR_DIR}/NAME.csr
    - a KEY in ${SSL_KEY_DIR}/NAME.key.new
...
sed_key_cert_path_for_apache() {
    ...
    # Update cert
    local cert_path=$2
    local search_cert="^SSLCertificateFile.*$"
    local replace_cert="SSLCertificateFile ${cert_path}"
    local cert_updated=0
    if grep -qE "${search_cert}" "${vhost_full_path}"; then
        [ -w "${vhost_full_path}" ] || error "File ${vhost_full_path} is not writable"

        sed -i "s~${search_cert}~${replace_cert}~" "${vhost_full_path}"
        cert_updated=1
        debug "Certificate in ${vhost_full_path} has been updated"
    fi

    # Update private key
    local key_path="${SSL_KEY_DIR}/${vhost}.key"
    local search_key="^SSLCertificateKeyFile.*$"
    local replace_key="SSLCertificateKeyFile ${key_path}"
    local key_updated=0
    if grep -qE "${search_key}" "${vhost_full_path}"; then
        [ -w "${vhost_full_path}" ] || error "File ${vhost_full_path} is not writable"

        sed -i "s~${search_key}~${replace_key}~" "${vhost_full_path}"
        key_updated=1
        debug "Private key in ${vhost_full_path} has been updated"
    fi
...
sed_key_cert_path_for_nginx() {
    ...
    # Update cert
    local cert_path=$2
    local search_cert="^ssl_certificate[^_].*$"
    local replace_cert="ssl_certificate ${cert_path};"
    local cert_updated=0
    if grep -qE "${search_cert}" "${vhost_full_path}"; then
        [ -w "${vhost_full_path}" ] || error "File ${vhost_full_path} is not writable"
    ...
    # Update key
    local key_path="${SSL_KEY_DIR}/${vhost}.key"
    local search_key="^ssl_certificate_key[^_].*$"
    local replace_key="ssl_certificate_key ${key_path};"
    local key_updated=0
    if grep -qE "${search_key}" "${vhost_full_path}"; then
        [ -w "${vhost_full_path}" ] || error "File ${vhost_full_path} is not writable"
...
main() {
    ...
    # update Nginx
    sed_key_cert_path_for_nginx "${VHOST}" "${LIVE_FULLCHAIN}"
